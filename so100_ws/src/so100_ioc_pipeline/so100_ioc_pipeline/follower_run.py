...
class FollowerRun(Node):
    def __init__(self):
        ...
        # State for velocity integration
        self.dq_cmd = np.array([0.0, 0.0])
    ...
    def tick(self):
        ...
        # LQR control: u = -K*x (u is acceleration)
        u = -self.K @ x
        u = self.scale * u
        
        # Integrate acceleration to get velocity command
        # dq_cmd = dq_cmd + u*dt
        self.dq_cmd = self.dq_cmd + u.flatten() * self.dt
        
        # Limit velocity
        max_dq = 1.0  # rad/s
        self.dq_cmd = np.clip(self.dq_cmd, -max_dq, max_dq)
        
        # Integrate velocity to get position command
        # q_cmd = q + dq_cmd*dt
        qcmd = np.array([q1, q2]) + self.dq_cmd * self.dt
